...
_FLAGOS_ZERO_COPY_KV_RETURN = False


def enable_flagos_zero_copy_kv_return(enabled: bool) -> None:
    """Return resident K/V views instead of repacking both caches per layer."""

    global _FLAGOS_ZERO_COPY_KV_RETURN
    _FLAGOS_ZERO_COPY_KV_RETURN = bool(enabled)


def _flagos_pack_kv_for_return(
    key_states: torch.Tensor, value_states: torch.Tensor
):
    if _FLAGOS_ZERO_COPY_KV_RETURN:
        return (key_states, value_states)
    return torch.cat((key_states, value_states), dim=0)
...
def _make_causal_mask(
    ...
    mask = torch.full((tgt_len, tgt_len), torch.finfo(dtype).min, device=device,dtype=bool)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        #why avoid?
        # [MODIFIED] enable the returning of kv caches
        #print('llama attention')
        #print(key_states.shape)
        #print(value_states.shape)
        past_key_value = _flagos_pack_kv_for_return(key_states, value_states)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.hidden_size = config.hidden_size
        #MODIFIED: SPDA Attention to support the openvla.
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(
        ...
        if attention_mask is not None:
            ...
            #print(expanded_attn_mask.shape)
    ...
    def forward(
        ...
        #print('init next decoder cache',next_decoder_cache)
        ...
        # !!!
        # all_hidden_states += (hidden_states,)
        #print('next decoder cache')
        #print(len(next_decoder_cache))
        #print(next_decoder_cache[0].shape)
        #exit()
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        #print('outputs',outputs.past_key_values)
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        #print(position_ids.shape[1])
        cos, sin = self.rotary_emb(value_states, seq_len=position_ids.shape[1])
        ...
        if attention_mask is not None:
            causal_mask = causal_mask[:, :, :, : key_states.shape[-2]].bool()
