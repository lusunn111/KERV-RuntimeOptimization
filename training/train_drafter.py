"""Train the KERV drafter from frozen OpenVLA supervision."""

import deepspeed
import os
import argparse
import wandb
parser = argparse.ArgumentParser(description='Train the KERV drafter')
parser.add_argument('--local_rank', type=int, default=0,
                    help='local rank passed from distributed launcher')
parser.add_argument('--base-model', required=True, help='OpenVLA checkpoint directory')
parser.add_argument('--data-dir', required=True, help='Generated drafter training samples')
parser.add_argument('--output-dir', required=True, help='Directory for drafter checkpoints')
parser.add_argument(
    '--drafter-config',
    default=os.path.join(os.path.dirname(__file__), 'drafter_config.json'),
)
parser.add_argument('--epochs', type=int, default=200)
parser.add_argument('--save-every', type=int, default=10)
parser.add_argument('--wandb-project', default='KERV-Drafter')
parser.add_argument('--wandb-mode', default='disabled', choices=['disabled', 'offline', 'online'])
# Include DeepSpeed configuration arguments
parser = deepspeed.add_config_arguments(parser)
cmd_args = parser.parse_args()
basepath = cmd_args.base_model
cpdir = cmd_args.output_dir
tmpdir = cmd_args.data_dir
train_config = {
    "lr": 5e-5,
    "bs": 4,
    ...
    "residual": "true,norm",
    "max_len": 2048,
    "config_path": cmd_args.drafter_config,
    "b1": 0.9,
    "b2": 0.95,
    "grad_clip": 0.5,
}
...
import numpy as np
os.environ['WANDB_MODE'] = cmd_args.wandb_mode
deepspeed.init_distributed()
rank = torch.distributed.get_rank()
if rank == 0:
    import wandb

    wandb.init(project=cmd_args.wandb_project, config=train_config)
...
for epoch in range(num_epochs):
    ...
    should_save = (epoch + 1) % cmd_args.save_every == 0 or epoch + 1 == num_epochs
    if should_save:
